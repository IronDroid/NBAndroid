...
public class DevicesNode extends AbstractNode implements PropertyChangeListener {
  ...
  // TODO(radim): needs large refactoring. Either fix DalvikPlatformManager or use SdkManager
    ...
    private void updateDescription () {
        setDisplayName(NbBundle.getMessage(DevicesNode.class, "TXT_Devices"));
        String description;
        final AndroidDebugBridge debugBridge = AndroidDebugBridgeFactory.getDefault();
        broken = debugBridge == null;
        if (broken) {
            description = NbBundle.getMessage(DevicesNode.class, "HINT_DevicesBroken");
        }
        else {
            description = NbBundle.getMessage(DevicesNode.class, "TXT_Devices");
        }
        setShortDescription(description);
        setIconBaseWithExtension("org/netbeans/modules/android/core/resources/android.png");        //NOI18N
    }
