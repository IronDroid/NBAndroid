 ...
 */
public class NewAndroidTestProjectWizardIterator implements WizardDescriptor.ProgressInstantiatingIterator {

    static final String PROP_NAME_INDEX = "nameIndex";      //NOI18N

  public NewAndroidTestProjectWizardIterator() {
    // XXX may need to switch between app, test, and lib projects?
  }

  private WizardDescriptor.Panel[] createPanels() {
    return new WizardDescriptor.Panel[]{
          new PanelConfigureProject(true)
        };
  }

  private String[] createSteps() {
    return new String[]{
          NbBundle.getMessage(NewAndroidTestProjectWizardIterator.class, "LAB_ConfigureTestProject"),};
  }
  ...
  @Override
    ...
    String testedProject = (String) wiz.getProperty("testedProject");        //NOI18N
    ...
    String relativePathToMain = PropertyUtils.relativizeFile(dirF, new File(testedProject));
    if (relativePathToMain == null) {
      throw new IOException("Cannot create relative path from " + dirF.getAbsolutePath() + " to " + testedProject);
    }
    data.setMainProjectDirPath(relativePathToMain);

    handle.progress(NbBundle.getMessage(NewAndroidTestProjectWizardIterator.class, 
        "LBL_NewAndroidProjectWizardIterator_WizardProgress_CreatingProject"), 1);

    // TODO can we pass null aas activity entry?
    AndroidProjectUtil.create(data, pkgName, /*activity*/null);
    handle.progress(2);
    ...
    // TODO handle setAsMain property and set main project if requested
  ...
  @Override
  public String name() {
    return NbBundle.getMessage(NewAndroidTestProjectWizardIterator.class, "LAB_IteratorName", index + 1, panels.length);
  }
  ...
  static String getPackageName(String displayName) {
    ...
    return builder.length() == 0 ? NbBundle.getMessage(NewAndroidTestProjectWizardIterator.class, "TXT_DefaultPackageName") : builder.toString();
  }
}
