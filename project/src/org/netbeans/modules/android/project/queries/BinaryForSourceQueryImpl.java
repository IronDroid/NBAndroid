...
public class BinaryForSourceQueryImpl implements BinaryForSourceQueryImplementation {        
    ...
    public @Override Result findBinaryRoots(URL sourceRoot) {
        if (sourceRoot.equals(FileUtil.urlForArchiveOrDir(new File(project.getProjectDirectoryFile(), "src"))) ||
                sourceRoot.equals(FileUtil.urlForArchiveOrDir(new File(project.getProjectDirectoryFile(), "gen")))) {
            return new Result() {
