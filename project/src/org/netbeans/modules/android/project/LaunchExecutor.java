...
class LaunchExecutor {
  ...
  public void doLaunchAfterBuild(final String command, FileObject buildFo, ExecutorTask task) {
    if (!isLaunchingCommand(command)) {
      return;
    }
    ...
    final LaunchInfo launchInfo = new LaunchInfo(
        ...
        true,
        isDebugCommand(command), launchConfig,
        AndroidProjectUtil.parseProjectManifest(project));
    ...
    private LaunchInfo fixLaunchedAppName(LaunchInfo li, String testedAppName, String testedAppName2) {
      ...
      if ((li.packageFile == null || li.packageFile.isValid())) {
        ...
        if (apk != null && apk.isValid()) {
          return new LaunchInfo(apk, li.reinstall, li.debug, li.launchConfig, li.manifestData);
        }
    ...
    private static boolean isDebugCommand(String command) {
      return ActionProvider.COMMAND_DEBUG.equals(command);
    }

    private static boolean isLaunchingCommand(String command) {
      return ActionProvider.COMMAND_DEBUG.equals(command) ||
